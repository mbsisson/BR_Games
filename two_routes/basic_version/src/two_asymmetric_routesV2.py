...
def play_game(T, thetaBlue, thetaRed, hitOdds, delta=0):
    ...
    for t in range(1, T):
        Bloc = location[t-1, Blue]
        Rloc = location[t-1, Red]
        if reward[t-1] == 1:
            # Red did not strike or missed in previous period
            Bq = Bq1 if Bloc == routeI else Bq2
            location[t, Blue] = Bloc if choose_to_stay(Bq, delta) else other_route(Bloc)

            Rq = Rq1 if Rloc == routeI else Rq2
            location[t, Red] = Rloc if choose_to_stay(Rq, delta) else other_route(Rloc)

        else: 
            # Red struck in previous period
            Bp = Bp1 if Bloc == routeI else Bp2
            location[t, Blue] = Bloc if choose_to_stay(Bp, delta) else other_route(Bloc)

            Rp = Rp1 if Rloc == routeI else Rp2
            location[t, Red] = Rloc if choose_to_stay(Rp, delta) else other_route(Rloc)
